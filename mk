...
if [ $SYS = "Linux" ]; then
    DEFS="NVLinux"
    LIBRARIES="-L $PFM_LIB -lllz -lpfm -lproj -lnvutility -lgdal -lxml2 -lpoppler -lz -lGLU -lm"
    export LD_LIBRARY_PATH=$PFM_LIB:$QTDIR/lib:$LD_LIBRARY_PATH
else
    DEFS="NVWIN3X"
    LIBRARIES="-L $PFM_LIB -lllz -lpfm -lproj -lnvutility -lgdal -lxml2 -lpoppler -lm -liconv"
